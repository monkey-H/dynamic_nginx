...
typedef struct {
    ...
    ngx_array_t  *lengths;
    ngx_uint_t    retries;
} ngx_http_upstream_myhash_conf_t;
...
#endif
} ngx_http_upstream_myhash_peer_t;

typedef struct {
    ngx_uint_t                        number;
    ngx_uint_t                        total_weight;
    unsigned                          weighted:1;
    ngx_http_upstream_myhash_peer_t     peer[0];
} ngx_http_upstream_myhash_peers_t;

typedef struct {
    ngx_http_upstream_myhash_peers_t   *peers;
    uint32_t                          hash;
    ngx_str_t                         current_key;
    ngx_str_t                         original_key;
    ngx_uint_t                        try_i;
    uintptr_t                         tried[1];
} ngx_http_upstream_myhash_peer_data_t;


static ngx_uint_t ngx_http_upstream_get_hash_peer_index(
    ngx_http_upstream_myhash_peer_data_t *uhpd);
static void ngx_http_upstream_myhash_next_peer(ngx_http_upstream_myhash_peer_data_t *uhpd,
...
static void ngx_http_upstream_save_hash_peer_session(ngx_peer_connection_t *pc,
    void *data);
#endif
static char *ngx_http_upstream_myhash(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
static char *ngx_http_upstream_myhash_again(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
static ngx_int_t ngx_http_upstream_init_hash(ngx_conf_t *cf,
    ngx_http_upstream_srv_conf_t *us);
static ngx_uint_t ngx_http_upstream_myhash_crc32(u_char *keydata, size_t keylen);
static void *ngx_http_upstream_myhash_create_conf(ngx_conf_t *cf);


static ngx_command_t  ngx_http_upstream_myhash_commands[] = {
    { ngx_string("myhash"),
      NGX_HTTP_UPS_CONF|NGX_CONF_NOARGS,
      ngx_http_upstream_myhash,
      0,
      0,
      NULL },
    ...
    { ngx_string("hash_again"),
      ...
      ngx_http_upstream_myhash_again,
      0,
      0,
      NULL },
...
static ngx_http_module_t  ngx_http_upstream_myhash_module_ctx = {
    ...
    ngx_http_upstream_myhash_create_conf,    /* create server configuration */
...
ngx_module_t  ngx_http_upstream_myhash_module = {
    NGX_MODULE_V1,
    &ngx_http_upstream_myhash_module_ctx,    /* module context */
    ngx_http_upstream_myhash_commands,       /* module directives */
...
static ngx_uint_t
ngx_http_upstream_get_hash_peer_index(ngx_http_upstream_myhash_peer_data_t *uhpd)
{
    ngx_int_t   w;
    ngx_uint_t  i;
...
ngx_http_upstream_init_hash(ngx_conf_t *cf, ngx_http_upstream_srv_conf_t *us)
{
	...
	fprintf(stderr, "addr %s\n", inet_ntoa(((struct sockaddr_in *)server[i].addrs[0].sockaddr)->sin_addr));
	fprintf(stderr, "addr %d\n", (unsigned short)ntohs(((struct sockaddr_in *)server[i].addrs[0].sockaddr)->sin_port));
    ...
    peers = ngx_pcalloc(cf->pool, sizeof(ngx_http_upstream_myhash_peers_t)
            + sizeof(ngx_http_upstream_myhash_peer_t) * n);
    ...
    for (i = 0; i < us->servers->nelts; i++) {
	    ...
	    fprintf(stderr, "peer addr %s\n", inet_ntoa(((struct sockaddr_in *)server[i].addrs[j].sockaddr)->sin_addr));
#if (NGX_HTTP_HEALTHCHECK)
            if (!server[i].down) {
                health_index =
...
{
    ngx_http_upstream_myhash_peer_data_t     *uhpd;
    ngx_http_upstream_myhash_conf_t          *uhcf;
    struct sockaddr_in 			     *sin;
    u_char                                   *p;

    ngx_str_t val;

    uhcf = ngx_http_conf_upstream_srv_conf(us, ngx_http_upstream_myhash_module);

    if (ngx_http_script_run(r, &val, uhcf->lengths, 0, uhcf->values) == NULL) {
        return NGX_ERROR;
    }

    uhpd = ngx_pcalloc(r->pool, sizeof(ngx_http_upstream_myhash_peer_data_t)
            + sizeof(uintptr_t) *
                ((ngx_http_upstream_myhash_peers_t *)us->peer.data)->number /
                    (8 * sizeof(uintptr_t)));
    if (uhpd == NULL) {
        return NGX_ERROR;
    }
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "sockaddr sin %c", (char)p[0]);

    ngx_memcpy(uhpd->current_key.data, val.data, val.len);
    uhpd->current_key.len = val.len;
    uhpd->original_key = val;
    uhpd->hash = ngx_http_upstream_myhash_crc32(uhpd->current_key.data, uhpd->current_key.len);
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "upstream_myhash: hashed \"%V\" to %ui", &uhpd->current_key,
                   ngx_http_upstream_get_hash_peer_index(uhpd));
    uhpd->try_i = 0;

    /* In case this one is marked down */
    ngx_http_upstream_myhash_next_peer(uhpd, &r->upstream->peer.tries, r->connection->log);
    if ((ngx_int_t)r->upstream->peer.tries == -1) {
        return NGX_ERROR;
    }

    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "upstream_myhash: Starting with %ui",
                   ngx_http_upstream_get_hash_peer_index(uhpd));
...
{
    ngx_http_upstream_myhash_peer_data_t  *uhpd = data;
    ngx_http_upstream_myhash_peer_t       *peer;
    ngx_uint_t                           peer_index;

    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, pc->log, 0,
                   "upstream_myhash: get upstream request hash peer try %ui", pc->tries);
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_HTTP, pc->log, 0,
                   "upstream_myhash: chose peer %ui w/ hash %ui for tries %ui", peer_index, uhpd->hash, pc->tries);
    ...
    ngx_uint_t state)
{
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, pc->log, 0,
            "upstream_myhash: free upstream hash peer try %ui", pc->tries);
    ...
    if (state & (NGX_PEER_FAILED|NGX_PEER_NEXT)
        ...
        ngx_http_upstream_myhash_next_peer(uhpd, &pc->tries, pc->log);
        ngx_log_debug2(NGX_LOG_DEBUG_HTTP, pc->log, 0,
                       "upstream_myhash: Using %ui because %ui failed",
...
static void ngx_http_upstream_myhash_next_peer(ngx_http_upstream_myhash_peer_data_t *uhpd,
    ...
    while ((*tries)-- && (
       ...
       uhpd->hash += ngx_http_upstream_myhash_crc32(uhpd->current_key.data,
           uhpd->current_key.len);
       current = ngx_http_upstream_get_hash_peer_index(uhpd);
       ngx_log_debug2(NGX_LOG_DEBUG_HTTP, log, 0,
           "upstream_myhash: hashed \"%V\" to %ui", &uhpd->current_key, current);
   } 
}

/* bit-shift, bit-mask, and non-zero requirement are for libmemcache compatibility */
static ngx_uint_t
ngx_http_upstream_myhash_crc32(u_char *keydata, size_t keylen)
{
...
static char *
ngx_http_upstream_myhash(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    uhcf = ngx_http_conf_upstream_srv_conf(uscf, ngx_http_upstream_myhash_module);
...
static void *
ngx_http_upstream_myhash_create_conf(ngx_conf_t *cf)
{
    ngx_http_upstream_myhash_conf_t  *conf;

    conf = ngx_pcalloc(cf->pool, sizeof(ngx_http_upstream_myhash_conf_t));
    if (conf == NULL) {
        return NULL;
    }
...
static char *
ngx_http_upstream_myhash_again(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_http_upstream_srv_conf_t   *uscf;
    ngx_http_upstream_myhash_conf_t  *uhcf;
    ngx_int_t                       n;
